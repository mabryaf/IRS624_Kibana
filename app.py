...
def random():
    ...
    if random==True:
        ...
        query["query"]["bool"]["should"].append(random_query)
    if yeargte:
        year = {"range": { "year": { "gte": yeargte, "lte": yearlte}}}
        query["query"]["bool"]["filter"].append(year)
    if avg_vote:
    ...
    return render_template("results.html", q_exact=res['hits'], genres=input_genre, countries=input_country, langs=input_lang)
